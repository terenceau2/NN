...
model.add(Dense(50,activation='relu'))  #to check its weights, use model.layers[1].variables
...
def weighted_cross_entropy(y_true,y_pred):
    y_pred1=tf.math.add(y_pred,0.00001)
    log_y_pred=tf.math.log(y_pred1)
    ce=tf.math.multiply(y_true,log_y_pred)
    weighted_ce=tf.math.multiply(ce,tf.constant([5.0,1.0], dtype='float32'))
    weighted_ce=tf.reduce_sum(weighted_ce)
    return weighted_ce

"""
y_true=y_train[0]
y_pred=np.random.rand(10,2)
x = tf.Variable([0.5], dtype='float32')

with tf.GradientTape() as tape:
    y_true1 = y_true + x ** 2
    loss = weighted_cross_entropy(y_true1, y_pred)
...
class Custom_loss(tf.keras.losses.Loss):
  ...
  def call(self, y_true, y_pred):
    ...
    log_y_pred=tf.math.log(y_pred1)
    ce=tf.math.multiply(y_true,log_y_pred)
    weighted_ce=tf.math.multiply(ce,tf.constant([1.0,1.0], dtype='float32'))
    weighted_ce=tf.reduce_sum(weighted_ce)
    return weighted_ce
...
model.compile(optimizer=opt, loss=weighted_cross_entropy)
#model.compile(optimizer=opt, loss=Custom_loss()) #this is exactly the same with the above command of using weighted_cross_entropy
...
#img_file = './model_arch.png' #/Users/terenceau1/PycharmProjects/model_arch.png
...
#for the custom loss pos weight=5,1
3673 6327 0 3673
0.3673 1.0 0.537263219483654
Counter({'I': 10000})

#for the custom loss pos weight=2,1
1842 6025 1831 2144
0.23414262107537817 0.5014974135583992 0.3192374350086655
Counter({'I': 7867, 'O': 2133})


#for the custom loss pos weight=1,1
1100 5730 2573 1697
0.16105417276720352 0.29948271167982576 0.20946396267733028
Counter({'I': 6830, 'O': 3170})
...
351 4415 3322 2263
0.07364666386907259 0.09556221072692622 0.08318521151795236
Counter({'O': 5234, 'I': 4766})
